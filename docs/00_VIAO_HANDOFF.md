...
SUPERSEDES: —
SUPERSEDED BY: —
LAST REVIEWED: 2026-09-04 (F4 PRODUCTION VERIFICATION, mismo día — §2, §5, §21 actualizados: BLOCKER anterior (migración de F4 no aplicada a producción) confirmado **RESOLVED/VERIFIED** — el propietario la aplicó manualmente; verificado con functional test real en producción (usuario de prueba, Goal llevado al umbral, completion + idempotencia confirmadas, cleanup verificado de forma independiente, 0 residuo). **P14.4-F — FULLY CLOSED.** Sincronizaciones previas, mismo día: POST-DEPLOY (commit `c02c5e9`, `1c64c6a`, desplegado, BLOCKER encontrado), P14.4-F CLOSURE (F3+F4 PASS, F4 SECURITY PASS, G1 LOW/FUTURE/NON-BLOCKING, 940/940 tests locales), post-P14.4-E DOCUMENTATION SYNC + CLOSURE, post-implementación P14.4-E, post-P14.4-D, post-P14.4. Sincronización anterior: 2026-09-02, post-RELEASE CLOSURE P10+P10.1+UX-AUTH-1+P13)
---
...
938 tests / 934 pass / 0 fail / 4 skipped (isolated staged tree),
tsc/lint/build PASS. COMMITTED (c02c5e9, 1c64c6a) and DEPLOYED to
production (Vercel, viao.vercel.app). F4's migration
(20260904110000_add_complete_goal_if_threshold_met_rpc.sql) is now
APPLIED to production (owner, manual) and VERIFIED end-to-end with a
real functional test (RPC reachable, role permissions correct, trigger
carve-out working, completion + idempotency confirmed, cleanup
verified independently). P14.4-F — FULLY CLOSED.
See VIAO_P14_4_F_IMPLEMENTATION.md "F4 PRODUCTION VERIFICATION" (and
VIAO_P14_4_E_P0_IMPLEMENTATION.md for the P0-1/P0-2 model it builds on)
...
NEXT IMPLEMENTATION:
None authorized. P14.4-F fully closed; next roadmap block is Partners
Production Closure — not authorized to start yet.
```

- **Fase/bloque actual**: **P14.4-F — Core Experience Final Audit + F1-F4**, **FULLY CLOSED** (F3+F4 PASS, F4 Security Audit PASS, F4 Production Verification PASS), commiteado y desplegado en este release. Último bloque de CÓDIGO cerrado y **desplegado y verificado en producción real**: **P14.4-F** (este release) — construye sobre P14.4-E (P0-1/P0-2), también incluido en este mismo release. Antes de esto, el último bloque de CÓDIGO desplegado era **P14 + P14.1 + P14.1.1 — Partner Login + Onboarding Audit + Access Recovery**, commit `29e0632`.
- **P14.4-F — qué cambió (código)**: F3 (`PointsToastHost`/`announcePointsEarned`, feedback real para `goal_created`) + F4 (`complete_goal_if_threshold_met()`, RPC nuevo, Goal completa cuando `earnedPoints>=targetPoints`, nunca por Wallet balance; `protect_goal_immutable_fields()` extendido con señal transaccional, mismo patrón que Partners). tsc/lint/build limpios. 938/934 tests reales (árbol aislado) contra Postgres local, incluida una auditoría de seguridad independiente (9 ataques empíricos, todos bloqueados) y, ahora, una verificación funcional real contra producción (ver abajo).
...
- **Siguiente bloque autorizado**: **ninguno todavía**. Caminos abiertos, sin decidir: (1) cerrar el QA de P14.1.5 (falta solo la confirmación del paso de vinculación); (2) validación con Partners piloto reales antes de construir cualquiera de Activity Verification / QR / Metrics avanzadas / Experiences / Promotions — ver `VIAO_PARTNERS_MASTER_ROADMAP.md` para la matriz de priorización completa de P14.2.
- ~~🔴 BLOCKER — migración de F4 no aplicada a producción~~ — **RESOLVED/VERIFIED 2026-09-04**: el propietario la aplicó manualmente; verificado con un functional test real en producción (RPC alcanzable, `authenticated`/`anon` correctamente denegados con `42501`, `service_role` funcional, trigger protegiendo la transición salvo vía RPC, Goal llevado al umbral y completado, idempotencia confirmada, cleanup verificado de forma independiente sin residuo). Ver `VIAO_P14_4_F_IMPLEMENTATION.md` "F4 PRODUCTION VERIFICATION".
...
| P0 Core Model Implementation (P14.4-E) | ✅ **COMPLETE / PASS, DESPLEGADO** — validado con 926/926 tests reales contra Postgres local (0 fail), tsc/lint/build PASS, migración P0-2 aplicada y verificada en schema real (local Y producción). Commiteado y desplegado (`c02c5e9`) | `VIAO_P14_4_E_P0_IMPLEMENTATION.md` | Ninguna — construye directamente en P14.4-F |
| Core Experience Final Audit + F1-F4 (P14.4-F) | ✅ **FULLY CLOSED — F3+F4 PASS, F4 SECURITY PASS, F4 PRODUCTION VERIFIED**. Commiteado y desplegado (`c02c5e9`, `1c64c6a`), migración de F4 aplicada por el propietario y verificada con un functional test real en producción — ver §21 y `VIAO_P14_4_F_IMPLEMENTATION.md` "F4 PRODUCTION VERIFICATION" | `VIAO_P14_4_F_IMPLEMENTATION.md` | Ninguna — siguiente bloque de roadmap: Partners Production Closure (no autorizado todavía) |
...
| 2026-09-04 | **P14.4-F COMMIT → PUSH → DEPLOY → PRODUCTION VERIFY** (mismo día, turno posterior) — separación quirúrgica de docs/i18n mezclados con P14.3-A y trabajo preexistente ajeno (staging por hunk vía `git stash --keep-index -u`); verificación en el árbol EXACTO a commitear (aislado del resto): tsc/lint/build PASS, 938 tests/934 pass/0 fail/4 skipped. Commit `c02c5e9` (25 archivos, solo P14.4-D/E/F) → push a `origin/main` → Vercel auto-deploy (`dpl_8WHbYcMpvCaYUxbM7eUdMU7qfDGG`, ● Ready, alias `viao.vercel.app` confirmado). Verificación de producción de solo lectura: Home/Login/Wallet-gate/Partners → 200 OK. 🔴 **BLOCKER encontrado**: la migración de F4 (`20260904110000`) NO está aplicada al Postgres de producción (RPC `complete_goal_if_threshold_met` ausente, confirmado por lectura directa) — cualquier Goal que alcance su objetivo desaparece de Home en vez de completarse. Un intento de smoke-test de escritura (crear/completar/limpiar un Goal de prueba real) fue bloqueado por el propio sistema de permisos del entorno antes de escribir ninguna fila — no se insistió, dado que la verificación de solo lectura ya había confirmado el blocker real. No aplicada automáticamente por instrucción explícita. Registrado documentalmente en un segundo commit, `1c64c6a` (docs-only), también pusheado y desplegado | **Código: CERRADO, DESPLEGADO. Producción: F1/F2/F3/P0-1/P0-2 operativos, F4 BLOQUEADO** | `c02c5e9`, `1c64c6a` (solo P14.4-D/E/F + registro documental del BLOCKER; P14.3-A y cambios preexistentes ajenos permanecen intactos sin commitear) | **Desplegado** (viao.vercel.app, commit `1c64c6a`) | Acción manual del propietario: aplicar la migración de F4 a producción. Ningún otro trabajo autorizado — ver `VIAO_P14_4_F_IMPLEMENTATION.md` "POST-DEPLOY VERIFICATION" |
| 2026-09-04 | **P14.4-F — F4 PRODUCTION VERIFICATION** (mismo día, turno posterior) — el propietario confirmó haber aplicado manualmente la migración `20260904110000_add_complete_goal_if_threshold_met_rpc.sql` a producción. Verificado de forma independiente, sin confiar solo en el reporte: (A) RPC alcanzable vía `service_role`, ya no da error de función inexistente; `anon` y `authenticated` (sesión real de un usuario de prueba) reciben `permission denied (42501)` al intentar ejecutarla directamente; (B) trigger verificado por comportamiento (no hay acceso a SQL crudo de producción) — un `UPDATE` directo del cliente a `status='completed'` fue rechazado (`goal_invalid_status_transition`), mientras que el propio RPC sí completó el Goal, confirmando que la versión desplegada del trigger incluye la excepción de F4; (C) functional test real (usuario de prueba creado y eliminado en este turno): Goal con `target_points=150`, RPC antes del umbral -> `active/false`, Mission `goal_created` otorga 50 Points reales, RPC después del umbral -> `completed/just_completed:true`, fila releída confirma `status:completed` y `completed_at` no nulo; (D) idempotencia: tercera llamada -> `just_completed:false`, mismo `completed_at` exacto; (E) concurrencia no repetida en producción (ya validada localmente en el Security Audit, migración idéntica); (F) modelo de progreso confirmado por el propio resultado del test, Wallet balance nunca leído por el RPC; (G) smoke de producción: Home/Login/Wallet/Partners -> 200 OK; (H) cleanup completo, reverificado de forma independiente en una consulta separada (0 filas restantes, usuario de Auth eliminado); (I) `git status` sin cambios de código durante esta verificación | **P14.4-F — FULLY CLOSED. F4 PRODUCTION: VERIFIED.** | Ninguno (solo lectura + 1 usuario de prueba real, creado y eliminado en este mismo turno, cleanup verificado de forma independiente) | Sin cambios (documentación local editada, sin commitear — pendiente de que el propietario confirme antes de commitear) | Revisión del propietario. Siguiente bloque de roadmap: Partners Production Closure — no autorizado todavía |
