...
SUPERSEDES: —
SUPERSEDED BY: —
LAST REVIEWED: 2026-09-04 (POST-DEPLOY, mismo día — §2, §5, §21 actualizados: P14.4-D/E/F commiteado (`c02c5e9`), pusheado y desplegado a producción (Vercel, ● Ready, alias viao.vercel.app confirmado). 🔴 BLOCKER encontrado en verificación de producción: la migración de F4 (`20260904110000_add_complete_goal_if_threshold_met_rpc.sql`) NO está aplicada al Postgres de producción — Goal Completion queda bloqueado ahí hasta aplicarla manualmente; NO aplicada automáticamente por instrucción explícita. Sincronizaciones previas, mismo día: post-P14.4-F CLOSURE (F3+F4 PASS, F4 SECURITY PASS, G1 LOW/FUTURE/NON-BLOCKING, 940/940 tests locales), post-P14.4-E DOCUMENTATION SYNC + CLOSURE, post-implementación P14.4-E, post-P14.4-D, post-P14.4. Sincronización anterior: 2026-09-02, post-RELEASE CLOSURE P10+P10.1+UX-AUTH-1+P13)
---
...
tsc/lint/build PASS. COMMITTED (c02c5e9) and DEPLOYED to production
(Vercel, viao.vercel.app). BLOCKER: F4's migration
(20260904110000_add_complete_goal_if_threshold_met_rpc.sql) is NOT
applied to production Postgres yet — Goal Completion is blocked there
until the owner applies it manually. F1/F2/F3/P0-1/P0-2 unaffected.
See VIAO_P14_4_F_IMPLEMENTATION.md "POST-DEPLOY VERIFICATION" (and
...
NEXT IMPLEMENTATION:
None authorized. Owner must apply F4's migration to production
Postgres manually (BLOCKER above) before Goal Completion works there.
```
...
- **🔴 BLOCKER, pendiente real, no de código**: la migración de F4 (`20260904110000_add_complete_goal_if_threshold_met_rpc.sql`) NO está aplicada al Postgres de **producción** — confirmado por lectura directa (PostgREST no encuentra el RPC). Efecto real: cualquier usuario que alcance el objetivo de Points de su Goal activo lo ve desaparecer de Home en vez de completarse. Acción manual pendiente del propietario — no aplicada automáticamente por instrucción explícita. Ver `VIAO_P14_4_F_IMPLEMENTATION.md`.
...
| Core Experience Final Audit + F1-F4 (P14.4-F) | ✅ **CERRADO en código — F3+F4 PASS, F4 SECURITY PASS. Commiteado y desplegado (`c02c5e9`)**. 🔴 **F4 BLOQUEADO en producción real**: migración `20260904110000` no aplicada — ver §21 y `VIAO_P14_4_F_IMPLEMENTATION.md` "POST-DEPLOY VERIFICATION". F1/F2/F3/P0-1/P0-2 sí operativos en producción | `VIAO_P14_4_F_IMPLEMENTATION.md` | **Acción manual del propietario**: aplicar la migración de F4 a producción |
