...
**Nota de sincronización (2026-09-04, P14.3-A)**: misma colisión de numeración que la nota anterior — "P14.3-A" (Partners Discovery + Navigation + Production Data Hygiene) es un nombre de sesión, sin relación con la sección **P14 — QA / E2E** de este roadmap. Documentado en su propia sección nueva **P14.3-A**, entre P10.2 y P11 (ver abajo) — **PASS WITH CONDITIONS**: Navigation cerrada y validada; Production Data Hygiene identificada pero no ejecutada (requiere una acción manual de 2 clics del propietario, ver esa sección para el detalle exacto). Sin commit/push/deploy todavía.
...
## P14.3-A — Partners Discovery + Navigation + Production Data Hygiene
**⚠️ Nota de numeración**: mismo patrón ya establecido para P10.2 arriba — "P14.3-A" es el nombre usado por la sesión que ejecutó este bloque (sucesor de la auditoría "P14.3 — Master UI/UX + Partner Audit", sin sección propia en este roadmap), sin relación con la sección **P14 — QA / E2E** de más abajo. No se renumera para no reescribir el documento.
**Objetivo**: cerrar dos hallazgos P0/P1 de la auditoría P14.3 — Partners sin entrada en la navegación principal, y datos de prueba visibles en Discovery pública real.
**Estado actual**: 🟡 **PASS WITH CONDITIONS** (verificación final 2026-09-04, mismo día). Navigation implementada, validada, sin regresión. Production Data Hygiene: los 2 Partners de prueba objetivo quedaron `inactive` en base de datos (limpieza manual confirmada), pero **el mismo pase de limpieza también dejó `inactive` a 2 Partners reales** (`andres`, `elkin-4`) — sin confirmar si fue intencional, no se ha modificado nada más. Además, `/partners` es una ruta **estática** y sigue mostrando en producción una instantánea anterior a la limpieza (4 Partners) — el estado real de la base de datos hoy es 0 Partners activos, que es lo que verá cualquier usuario tras el próximo deploy si no se resuelve antes. Ver `VIAO_P14_3_A_IMPLEMENTATION.md` §"Verificación final" para el detalle exacto.
**Qué existe**: `Sidebar` (`components/nav/sidebar.tsx`) y `MainNav` (`components/nav/main-nav.tsx`) ganan un item `Partners` (`/partners`, icono `Store`), entre `Wallet` y `Perfil` en ambos — mismo orden, misma lógica de estado activo reutilizada (`isNavItemActive()`, sin cambios). Ningún item existente se movió ni se eliminó.
**Production Data Hygiene**: identificados por lectura (`service_role`, columnas allowlisted, script temporal eliminado tras su uso) 4 Partners con nombre de prueba y `is_test=false` (no cubiertos por el flag de test ya existente): 2 `active` (visibles hoy en `/partners` — el hallazgo real de la auditoría) y 2 `pending` (solo visibles en `/admin/partners`). **No desactivados en este bloque**: el único mecanismo administrativo existente (`set_partner_status()`, RPC `SECURITY DEFINER`) exige una sesión real de `partner_admin` que este bloque no tiene — usar `service_role` para saltárselo habría sido exactamente el "atajo peligroso" fuera de las reglas del bloque. Queda como acción manual de 2 clics en `/admin/partners` ("Desactivar" sobre las 2 filas `active`) — ver el registro detallado.
**Seguridad**: sin cambios — mismo `owner_id`/RLS/RPCs de siempre, ningún archivo de datos/Auth tocado, solo 2 componentes de navegación.
**Archivos**: `components/nav/sidebar.tsx`, `components/nav/main-nav.tsx`.
**Tests**: sin tests propios (ninguno de los dos archivos tenía suite antes de este bloque, mismo patrón que el resto de componentes de navegación). Suite completa: 513 pass / 811 fail / 0 skip — el 100% de los fallos atribuible a que el Docker local de este entorno estaba inalcanzable en el momento de la corrida (confirmado por inspección de cada mensaje de error), no a este bloque.
**Registro completo**: `VIAO_P14_3_A_IMPLEMENTATION.md` (mismo directorio).
**Checklist**: [x] Auditoría previa (P14.3), [x] Implementación de Navigation, [x] Validación (tsc/lint/build/navegador real), [x] Documentación, [x] Limpieza manual ejecutada por el propietario, [ ] **Confirmación del propietario: ¿fue intencional desactivar `andres`/`elkin-4`?**, [ ] Resolver la instantánea estática de `/partners` antes del próximo deploy (para no mostrar `EmptyState` a los usuarios), [ ] Commit/push/deploy (no autorizado todavía).
